...
| `cli` | `src/cli.rs` | Argument parsing (`--all`/`-a`, `--project`/`-p`, `--model <value>`, `--help`/`-h`, hidden `--print-list`); resolves the canonical launch dir. Hand-rolled — no `clap`, no arg-parsing dependency at all — so the walk is one `match` over an explicit iterator, and `--model` is the ONE flag that takes a value: it consumes the next argument verbatim (getopt-style, no `--model=<value>` form), never validates it (that value is claude's to judge, exactly as the picker's is), and fails soft to no override when nothing follows it, matching the unknown-flag arm's forgiving posture. It seeds `App::model_override` through the picker's own setter, so a launch flag and a picked row are one setting. The two scope flags are mutually exclusive in meaning but not in syntax — the LAST one on the command line wins (the plain single-pass assignment, so an alias carrying `-p` stays overridable by a trailing `-a`). `-a` carries a SECOND, orthogonal meaning that precedence does not touch: `Args::all_scope_enabled`, which is what puts the all scope on the `Ctrl-A` cycle and is the only route to it. |
...
| `store::preview` | `src/store/preview.rs` | Transcript → `RenderedPreview` (styled ratatui `Text` + clickable `LinkRegion`s + per-turn `MarkerLine`s), self-contained markdown pass. A `● claude` marker carries up to four DIM annotations, in order: the bound-agent handle, the answering model (family + dotted version, via `model_label`, which `send` shares), the effort level (`record_effort`), and the time — see [DOMAIN.md](DOMAIN.md#answering-model-storepreview). The markers are the turn marker `Line`s captured during the SAME pass, addressed in the same content-row space as a link region and rebased by the same running line offset, so the pinned banner can show the marker of whichever turn the viewport's top row lands on without deriving a second copy of that text. |
...
| `agents` | `src/agents.rs` | Agent detection via `claude agents --json`, read TWO ways through ONE fail-soft parser. `reported_agents` (`--all`, polled off-thread every `watch::AGENTS_REFRESH`, 5s, skipped while the board is idle — see [DOMAIN.md](DOMAIN.md#why-the-gate-does-not-read-the---all-map)) is the DISPLAY signal: `classify` buckets each qualifier into an `AgentActivity` that the list row's translated qualifier (via `qualifier_copy`, which the pinned preview banner reaches ONLY through its fallback for a transcript with no marker at all), list-badge color and pulse (which alternates that color, never the glyph) derive from. `live_agents` (bare, NO `--all`) is the HAND-OFF signal, taken one-shot at EVERY hand-off — the resume/attach gate and the `Ctrl-R`/`Ctrl-K` gates alike — and its records also carry the short job `id` that `claude attach`/`claude stop` take. Why the two readings must never be merged, and why an authoritative decision never reads the polled map, is [DOMAIN.md](DOMAIN.md#why-the-gate-does-not-read-the---all-map) — do not restate it here. Framework-free: it interprets the value set, while the color it maps to is the view's call. |
...
| `model_aliases` | `src/model_aliases.rs` | The `--model` alias set the INSTALLED `claude` accepts, read out of that binary at RUNTIME so the `Ctrl-X m` picker is not a compile-time capture that rots on every claude release (`claude --help` cannot be the source — it names three of the nine). Split exactly like `agents`: pure `parse_model_aliases` is the SINGLE interpreter of the byte format (a flat, minified array literal), which selects by CONTENT ANCHOR — `opusplan`, carried by no sibling array — with longest-wins only as a deterministic tie-break, and NEVER by the minified identifier holding the array, which is regenerated on every build. The impure `installed_model_aliases` walks `$PATH`, canonicalizes the hit (that entry is a symlink into a versioned store, so without canonicalizing there is no saying WHICH version was read) and chunk-reads the ~290 MB non-UTF-8 file with an overlap window sized from the same cap the extractor enforces, so no match can straddle a chunk boundary unseen. FAIL-SOFT to an EMPTY vector on every branch — no `PATH`, no binary, a failed canonicalize, an unreadable file, a read error, no match — which callers read as "keep the seed", never as an error to handle. Uses NEITHER `nucleo` NOR `memchr` (NUCLEO ISOLATION) and caches in a process-lifetime `OnceLock` with no disk write at all (SNAPBACK-OWNED STATE); both rule conflicts are resolved and argued in the module's own docs rather than by amending a rule. A plain blocking fn in `agents::reported_agents`' shape — it knows nothing about threads or events, and must never be called on the render path. |
| `watch` | `src/watch.rs` | Debounced FS watcher + `EventLoop` that merges input/watcher/tick/agents onto one channel, plus the ONE-SHOT `--model` alias probe (`spawn_model_alias_probe` over `spawn_model_alias_thread`), a thread that delivers a single `AppEvent::ModelAliases` and then ends — a seam that takes the probe as a PARAMETER, so the suite states an answer instead of walking a 290 MB binary. The watcher classifies each debounce batch against BOTH halves of the shape rule `store::discover` owns — `store_depth` for WHICH LEVEL a path sits at, `is_session_path` for whether it is a session — and reloads only when the batch is not provably irrelevant (`classify_watch_path` / `batch_needs_reload`), so an irrelevant write elsewhere in the store root never triggers a reload. The agents poller backs off to `AGENTS_REFRESH` (5s) and skips the `claude agents` shell-out once the board-activity stamp is older than `AGENTS_IDLE_AFTER` (60s) — see [DOMAIN.md](DOMAIN.md#reported-agents-srcagentsrs). |
| `resume` | `src/resume.rs` | Resume/fork/attach/new-session hand-off: re-read authoritative parts (or, for a new session, gate on the launch dir + optional `--agent <name>`), existence gate, spawn `claude`, return. Resume/fork/new also carry the board's sticky `--model` override (one shared `push_model_flag`; `build_attach_argv` takes no model at all, so Attach is excluded structurally). Each `Ready` carries its own non-zero hint, picked by `nonzero_hint_for`: the neutral resume-vs-new-session wording normally, or `MODEL_NONZERO_HINT` — which NAMES the model as a likely cause — whenever this invocation actually emits the flag. |
...
Five producers merged onto one `mpsc` channel of `AppEvent` — four recurring, and
one single-shot probe:
...
5. **Model-alias probe** — a ONE-SHOT, not a poller, started once per board
   session beside the agents poller (`EventLoop::spawn_model_alias_probe`). It
   reads the installed `claude`'s `--model` alias set off the UI thread
   (`model_aliases::installed_model_aliases`), delivers exactly one
   `AppEvent::ModelAliases`, and its thread ends. Sending once and returning is
   why it needs no shutdown flag: the bounding problem the flag exists for belongs
   to LOOPING producers that can accumulate one thread per resume round trip, and
   a thread that cannot loop cannot accumulate. Nothing waits on it — the
   `Ctrl-X m` picker draws its compile-time seed until the answer lands, and keeps
   the seed if it never does — which is what lets a scan measured at ~125 ms
   release / ~2.2 s debug sit on the same channel as the render loop. Per BOARD
   SESSION rather than per process on purpose: the scan itself is memoized in a
   `OnceLock`, so a later session re-delivers rather than re-reads, and
   re-delivering is what stops a probe still in flight across a resume hand-off
   from losing its answer to the receiver that hand-off dropped.
...
- a confirmed background-agent launch (`Enter` in the new-session draft pane) fires
  `send::spawn_bg_launch` for `claude [--agent <name>] [--model <alias>] --bg <prompt>`, delivering
