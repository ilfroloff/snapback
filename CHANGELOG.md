...
- delete a whole search term per press, not one character
- add Ctrl-T/Ctrl-E as preview top/bottom shortcuts

### Fixed

- *(tui)* delete one search term per press on a very long query
- keep a long search query and its caret on screen
- stop a typed search term from quitting the board

### Other

- record the search row's render ceiling and the Alt-key trade-off
- close the key-doc sync rule's own gap in its surface list
