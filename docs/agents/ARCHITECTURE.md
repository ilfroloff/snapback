...
| Grapheme clusters | `unicode-segmentation` | `=1.13.3` | extended cluster boundaries so NEITHER per-span split ever cuts a cluster — the search-match highlight's (`match_runs` in `src/tui/view.rs`) nor the table cell wrap's (`wrap_span_cells` in `src/store/preview.rs`): `Line::width` sums a CONTEXTUAL width PER SPAN, so severing an emoji from its VS16 or skin-tone modifier changes the summed width of unchanged text and desyncs the preview's cached widths and wrapped-row count from the painted line; already transitive via `ratatui-textarea` / `ratatui-core` / `unicode-truncate` |
| Display width | `unicode-width` | `=0.2.2` | terminal COLUMN count (not char count) on BOTH sides of the preview — the table cell layout's (`store::preview`'s markdown cells, so inline markers and CJK/emoji keep columns aligned) and the link hit-test's (`src/tui/view.rs`), which must answer in the very columns `store::preview` recorded a `LinkRegion` in: `region_char_positions` walks those columns back to chars with TWO accumulators, per whole SPAN outside (the unit `preview` summed) and per CLUSTER inside (the finest a region boundary can land on), because the fold is CONTEXTUAL and the two sums are not the same function; `region_paints_cell` then measures a marked cell's left neighbour against `WIDE_GLYPH_COLUMNS` to tell a wide glyph's unpainted right half from a column nothing painted at all; already transitive via `ratatui` |
...
line), and the CELL inside that one line resolved EXACTLY too, by re-rendering the
line with a candidate `LinkRegion` marked and reading the clicked cell back out of
an in-memory `Buffer` (`view::region_paints_cell`) rather than by modelling where
the private wrapper broke — against the cached `LinkRegion`s, and a hit is opened
off the render loop via `resume::open_url` AND reported as a transient
`opening <url>` status, which is the only thing separating a missed hit from an
opener that failed silently. A MISS writes no status, so it cannot evict an unread
sticky refusal; an unwritten cell is admitted only as a wide glyph's second half,
never as the blank tail of a row, so a click on empty space opens nothing.
