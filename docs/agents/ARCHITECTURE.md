...
left-click in the preview is mapped screen→content by `view::link_at` — the ROW
resolved EXACTLY through the same cached per-line prefix map the windowed draw
...
private wrapper will not say where inside a line it broke — against the cached
