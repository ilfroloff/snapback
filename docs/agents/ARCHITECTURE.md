...
transcript minds (the watcher reload covers it), and a lost `CopyFinished`
strands nothing either, since no status claims a copy before its result lands.
The draft card is different: it is UI state waiting on its event, so it cannot
be left to one that may never arrive:
