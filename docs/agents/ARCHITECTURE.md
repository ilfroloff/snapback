...
| Row-label highlight | `nucleo` | `=0.5.0` | isolated in `src/search.rs`; backs the row LABEL's highlight only — NOT the filter, and NOT the preview marks |
| Substring filter | `memchr` | `=2.8.2` | SIMD `memmem`; IS the per-keystroke filter, answering membership per atom, and marks the preview per atom too (`src/search.rs`); already transitive via `nucleo`/`serde_json` |
...
| `search` | `src/search.rs` | The **only** place `nucleo` or `memchr` is called: substring index, incremental re-filter, highlight seam. Per keystroke the filter answers MEMBERSHIP with `memchr::memmem` over prebuilt haystacks — no nucleo, no UTF-32 conversion, no ranking (`App::order_filtered` owns display order). Smart case is decided per ATOM, selecting the cased or lowercased haystack. TWO marking seams, split by what the surface IS: nucleo backs `match_indices`, the list row's LABEL — one string, matched as a whole, which is why the row is there at all — while `atom_match_positions` marks the preview's rendered lines PER ATOM through the filter's own memmem finders, because the transcript is many strings and the filter admitted it on atoms occurring ANYWHERE in it. The preview asks per RENDERED line rather than mapping a `content_index` position into the pane, which is not a thing that can be done (see [DOMAIN.md](DOMAIN.md#a-content-index-position-never-projects-into-preview-coordinates)). |
